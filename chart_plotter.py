...
def plot_combined_chart(df, stock_id, stock_name, show_ma_dict, k_line_type="一般K線"):
    ...
    fig.update_layout(
        ...
        modebar=dict(orientation='v', bgcolor='rgba(0,0,0,0.35)', color='#cccccc', activecolor='#7B68EE'),
        xaxis=dict(
        ...
        legend=dict(orientation="h", y=1.03, x=0.5, xanchor="center", font=dict(color='white', size=13)),
        autosize=True,
        dragmode='zoom'
    )
