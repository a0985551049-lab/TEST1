...
class StockDataLoader:
    ...
    def get_combined_data(_self, stock_id, days, use_adjusted=True):
        ...
        try:
            ...
            # ========== 0. 先查詢市場別，決定正確的 Yahoo 代碼後綴 ==========
            # 上市(twse) -> .TW，上櫃(tpex) -> .TWO；其他類型(如興櫃)Yahoo 通常查不到，直接跳過 Yahoo
            # 這一步很重要：股票代碼加錯後綴會讓 yfinance 對不存在的代碼發出請求，
            # 除了拿不到資料，某些情況下還可能連帶造成底層網路函式庫不穩定當機。
            stock_name = stock_id
            yf_suffix = None
            try:
                ...
                if not stock_info_all.empty:
                    ...
                    if not match_info.empty:
                        row0 = match_info.iloc[0]
                        stock_name = row0.get('stock_name', stock_id)
                        market_type = str(row0.get('type', '')).lower()
                        if market_type == 'twse':
                            yf_suffix = '.TW'
                        elif market_type == 'tpex':
                            yf_suffix = '.TWO'
                        # 其他類型（如興櫃 emerging）保持 None，跳過 Yahoo，只用 FinMind 資料
            except Exception:
                pass
            ...
            if df is None:
                ...
                if df_price.empty:
                    if not yf_suffix:
                        return None, "❌ 查無資料（此股票可能為興櫃或代碼有誤，且 FinMind 也查無資料）", None
                    # Yahoo 備援
                    yf_symbol = f"{stock_id}{yf_suffix}"
