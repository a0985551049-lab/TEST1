...
with st.sidebar.expander("🔑 AI 設定", expanded=True):
    ...
    with st.expander("⚙️ 進階：提高 FinMind 資料額度（選用）"):
        st.caption(
            "個股分析會用到多個 FinMind 資料集（股價、法人、融資、大戶持股比等），"
            "免費額度為 300次/小時。若經常密集使用容易額度用盡導致部分圖表顯示不出來，"
            "可至 [FinMind 官網](https://finmindtrade.com/) 免費註冊並取得 token，"
            "在 `secrets.toml` 加入一行：\n\n"
            "`FINMIND_TOKEN = \"你的token\"`\n\n"
            "額度即可提高到 600次/小時，不設定也能正常使用（走匿名額度）。"
        )
...
# ── 頁面狀態追蹤：讓「目前是否在個股分析頁」變成持續性狀態，不會因為使用者
#    在個股頁面上點擊其他元件（例如展開/收合區塊）而意外跳回首頁 ──
if "viewing_stock" not in st.session_state:
    st.session_state["viewing_stock"] = False

if run_analysis and stock_id:
    st.session_state["viewing_stock"] = True
    st.session_state["viewing_stock_id"] = stock_id
        ...
        if line.startswith('|'):
            ...
            if not in_table:
                ...
                for c in cells:
                    ...
                    if '族群分化' in c:
                        color = '#2c7fb8'
                    elif '似有鋪墊' in c or '初步布局' in c or '初步流入' in c:
                        color = '#c9990b'
                    elif '獲得青睞' in c or '流入' in c or '墊高' in c or '多頭' in c or '強勢' in c or c.startswith('+'):
                        color = '#d62839'
                    elif '遭獲利了結' in c or '流出' in c or '探底' in c or '空頭' in c or '弱勢' in c or c.startswith('-'):
                        color = '#2a9d3e'
                    td_cells.append(
                        f"<td style='padding:9px 14px;border:1px solid #ece9f7;"
                        f"color:{color};font-size:15px;line-height:1.7'>{c}</td>"
    ...
    for raw in cleaned_lines:
        ...
        if line.startswith('|'):
            ...
            if not in_table:
                ...
                for c in cells:
                    color = '#333333'
                    if '板塊內部分化' in c or '族群分化' in c:
                        color = '#2c7fb8'
                    elif '似有鋪墊' in c:
                        color = '#c9990b'
                    elif '獲得青睞' in c or '流入' in c or '多頭' in c or '強勢' in c or '攻勢' in c or c.startswith('+'):
                        color = '#d62839'
                    elif '遭獲利了結' in c or '流出' in c or '空頭' in c or '弱勢' in c or '守勢' in c or c.startswith('-'):
                        color = '#2a9d3e'
...
if st.session_state.get("viewing_stock") and st.session_state.get("viewing_stock_id"):
    stock_id = st.session_state["viewing_stock_id"]
    loader = StockDataLoader()
    
    k_type = "一般K線(未還原)" if use_normal else "還原K線"
    with st.spinner(f"🔄 正在載入 {stock_id} 數據 ({k_type})..."):
        df, error, stock_name, data_warnings = loader.get_combined_data(stock_id, days, use_adjusted)
    
    if error:
        st.error(error)
        if st.button("🔙 返回首頁", key="btn_back_on_error"):
            st.session_state["viewing_stock"] = False
            st.rerun()
        st.stop()
    ...
    with col_back:
        if st.button("🔙 返回首頁", use_container_width=True, key="btn_back_top"):
            st.session_state["viewing_stock"] = False
            st.rerun()
    with col_title:
    ...
    if 'institutional_rate_limit' in data_warnings or 'margin_rate_limit' in data_warnings:
        st.warning(
            "⚠️ FinMind API 額度已用盡（免費額度 300次/小時），本次籌碼/融資資料可能無法完整顯示。"
            "可稍後按「重新分析」再試一次；若經常需要密集使用，"
            "可至 [FinMind 官網](https://finmindtrade.com/) 免費註冊取得 token，"
            "在 App 的 Secrets 設定 `FINMIND_TOKEN` 後額度可提高到 600次/小時。"
        )
    elif 'institutional_empty' in data_warnings or 'margin_empty' in data_warnings:
        st.info("ℹ️ 本次未能取得外資/投信/融資等籌碼資料（FinMind 該股票的資料可能暫時延遲或空窗），圖表可能顯示為空，可稍後重試。")
    ...
    if df_holding is not None and not df_holding.empty:
        ...
        st.info(f"ℹ️ 大戶持股比例：{holding_err}")
    ...
    <div style='background: #ffffff;
        ...
        <div style='color:#555555; font-size:15px; line-height:1.8; margin-top:10px;
            ...
            再以股價表現為主軸分析各大類股輪動狀況，找出獲利了結、獲得青睞、似有鋪墊的族群及其原因。
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if run_market_flow:
        if not api_key:
            ...
            with st.spinner("📡 AI 正在搜尋近30日台股市場動態，分析類股輪動狀況（約 30~60 秒）..."):
    ...
    <div style='background: #ffffff;
        ...
        <div style='color:#555555; font-size:15px; line-height:1.8; margin-top:10px;
            ...
            並解讀 CPI、非農就業、失業率、FED 利率決策等總經數據，以股價表現為主軸研判美股板塊輪動（科技 vs 傳產/金融）及攻守勢。
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if run_us_market:
        if not api_key:
            ...
            with st.spinner("🌐 AI 正在搜尋近30日美股總經數據，分析板塊輪動狀況（約 30~60 秒）..."):
