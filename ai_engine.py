...
### 第二節：近30日類股資金流向地圖（至少900字）

**重要方法論：判斷資金流入/流出時，請評估「資金水位的變化趨勢」，而不是只看單日或單期的買賣超金額大小。**
具體來說：
- 請比較近30日每週（或每3~5個交易日為一區間）外資／投信買賣超金額的「累積水位」如何變化，
  是持續墊高（水位向上）、持續探底（水位向下）、還是高檔鈍化／低檔鈍化（水位持平但已達相對極端）。
- 一天的大量買超若只是相對於前一週持續探底的水位小幅反彈，不應直接判定為「資金流入」；
  需說明該水位目前處於近30日的相對高檔、中段還是低檔位置。
- 請具體寫出水位變化的方向與位置，例如：「外資買超金額連續5日墊高，水位已由近30日低點回升至中高檔」
  或「投信買超金額雖單日創高，但仍低於近30日平均水位，尚未脫離弱勢區間」。

**極重要：嚴禁把「產業面/消息面利多」直接當作「資金流向的證據」。**
營收創高、法說會樂觀、產業趨勢向好，這些是「基本面利多」，不等於「近期籌碼實際流入」——
族群基本面轉佳時，常常反而伴隨「利多出貨」或「族群內表態分化」（部分個股遭外資獲利了結），
兩者必須分開檢視，不能因為某類股有基本面題材，就直接推論資金正在流入。
下判斷前請先分別確認：（a）該類股／個股近期是否真的有外資或投信買超籌碼面證據，
（b）若只查得到營收/財報等基本面消息，找不到具體籌碼流向數據佐證，就不可以把它歸類為「資金流入」或
「確立流入」，應改標註為「基本面利多，籌碼面尚待確認」，並歸入「仍受關注類股」而非「資金流入類股」。

**極重要：判斷類股資金流向時，「必須」交叉檢查該類股內至少2~3檔權值或指標個股的個別狀況，
不可以只憑單一個股（尤其是最知名的龍頭股）的表現，就代表整個類股／族群的結論。**
- 常見的誤判陷阱：族群內某一檔指標股（例如AI伺服器概念股中的廣達）股價強勢、法人買超，
  但同族群的其他重要成員（例如緯創、鴻海）近期其實已經被外資／投信獲利了結、籌碼轉弱，
  這種情況下「不可以」只因為挑到那一檔強勢股，就把整個類股定調為「資金流入」或「確立流入」。
  必須誠實反映族群內部的分歧狀況。
- 具體做法：檢查你打算列為「代表個股」候選的2~3檔股票，各自近期的股價表現與外資/投信買賣超方向，
  然後依「多數個股方向是否一致」來決定類股的資金階段標籤：
  · 若2~3檔檢查對象「方向一致」（多數同步上漲＋買超，或多數同步下跌＋賣超），才可標為「確立流入」或「流出探底」
  · 若2~3檔檢查對象「方向分歧」（部分買超上漲、部分賣超下跌），應標為「族群分化」，並在描述中具體點出
    哪些個股偏多、哪些個股偏空，讓讀者看到族群內部並非鐵板一塊
  · 不可以為了敘事簡潔，只挑對結論有利的那一檔個股呈現，刻意忽略其他成員的背離表現

**極重要：「代表個股」欄位請列出2~3檔該類股的權值／指標個股（而非只列1檔），並各自標明近期表現與
外資/投信買賣超方向是否一致，讓讀者能看出這是「族群普遍現象」還是「單一個股表現」。**
- 若某類股被歸類為「資金流入」或「確立流入」，所列出的2~3檔代表個股中，多數應同步呈現買超或至少持穩，
  不能出現「類股寫資金流入，但列出的個股裡有一半其實股價下跌、且被外資賣超」這種自相矛盾的狀況。
- 若龍頭股本身其實正被外資獲利了結（股價下跌、籌碼賣超），但族群內其他個股籌碼偏多，應標為「族群分化」，
  並具體寫出「龍頭股（如X）近期反而遭獲利了結，但族群內Y、Z則持續獲外資買超，內部表態出現分歧」，
  不可只挑單一個股掩蓋族群整體的分歧狀況。
...
1. **資金流出類股**：哪些類股資金水位明顯向下探底？請列出前3~4名，每個至少80字，並說明水位變化的具體軌跡（不是只看單日金額）。

2. **資金流入類股**：哪些類股資金水位已經明顯、持續墊高，趨勢確立？請列出前3~4名，每個至少80字，並說明水位變化的具體軌跡。

3. **初步資金流入類股（外資試單布局訊號）**：這是獨立於上面「資金流入類股」的另一個類別，**務必找出至少2~3個類股填入，不可省略**。
   篩選重點是「水位剛開始從低檔或盤整區翻揚，買超天數雖不多但已連續出現、單筆金額不算誇張，
   看起來像是外資或投信剛開始小量試單、布局初期」的類股，而非已經噴出、漲幅已大的類股。
   請具體描述：（a）此類股先前的資金水位處於什麼位置（低檔盤整/持續流出後止穩等）、
   （b）近期出現什麼變化訊號讓它值得被歸類為「初步布局」（例如：連續3日轉為小額買超、
   買超筆數增加但金額尚溫和、法人持股比率出現緩步回升等）、
   （c）若這個初步訊號延續，後續可能演變成什麼樣的資金行為。
   這個類別的目的是幫助提前留意「還沒噴出、但法人可能已經開始卡位」的類股，請務必用審慎、
   非投資指示的語氣描述，並明確標註這僅是初步觀察訊號、不代表趨勢已確立。

4. **仍受關注類股**：哪些類股資金水位持續維持高檔或中性盤整，籌碼穩定？

5. **資金輪動路徑**：用「A類股 → B類股 → C類股」格式描述近30日資金水位輪動路徑，並試著標註目前資金正從哪個階段（流出/初步布局/確立流入）過渡到哪個階段。

接著請輸出一個「近30日類股資金流向總表」表格，**務必包含至少8行資料**，且必須涵蓋以上四種類別
（資金流出／資金流入／初步資金流入(外資布局)／仍受關注）都至少各出現1~2行，格式如下（每行都必須填入實際數據，嚴格按照此格式）：

| 類股名稱 | 資金階段 | 資金水位變化 | 主要驅動力/原因 | 代表個股（2~3檔，各自標明方向） |
|------|------|------|------|------|
| 半導體 | 確立流入 | 水位持續墊高 | AI需求強勁，且外資近30日持續買超 | 台積電+8%買超／聯發科+5%買超／聯電+3%買超，族群同步偏多 |
| 金融股 | 流出探底 | 水位持續探底 | 升息預期降溫，外資調節 | 富邦金-3%賣超／國泰金-2%賣超／中信金-1%賣超，族群同步偏空 |
| 電動車/綠能 | 初步布局 | 水位剛從低檔翻揚 | 外資近3日連續小額轉買超 | 某電動車概念股+2%轉買超，其餘成員仍觀望 |
| AI伺服器/ODM | 族群分化 | 個股表現不一致 | 廣達營收強勁+8%獲買超，但緯創、鴻海近期反遭外資獲利了結 | 廣達+8%買超／緯創-4%賣超／鴻海-2%賣超，族群內部分歧明顯 |

其中「資金階段」欄位請從「確立流入」「初步布局」「持平觀察」「流出探底」「基本面待確認」「族群分化」六種標籤中擇一填入。
「族群分化」代表類股內多檔指標個股方向不一致，不宜籠統歸類為單純流入或流出。
...
- 特別提醒本次「初步資金流入」類股名單，供後續持續觀察是否轉為確立趨勢
- 下月關鍵事件
- 風險提示
...
某板塊財報亮眼、營收成長，這是「基本面利多」，不等於「近期真的有資金流入」——
請優先搜尋 ETF 資金流向數據（如板塊 ETF 的申購/贖回、資金淨流入/流出報告）或法人持股變化等
具體籌碼面證據來判斷資金方向；若只查得到財報/獲利成長等基本面消息，找不到具體資金流向數據佐證，
應在該板塊描述中明確標註「基本面利多，資金流向尚待確認」，不要直接寫成「資金流入」。

**極重要：若描述中提到具體個股（例如AI伺服器概念股），「必須」交叉檢查該板塊內至少2~3檔重要成分股的
個別狀況，不可以只憑單一個股（尤其是最知名的龍頭股）的表現，就代表整個板塊的結論。**
常見誤判陷阱：某板塊內一檔指標股股價強勢、資金buying，但同板塊的其他重要成員其實已經被籌碼調節、
股價轉弱，這種情況下不可以只挑那一檔強勢股，就把整個板塊定調為「資金流入」。若板塊內多檔個股方向
不一致，請如實標註為「板塊內部分化」，並具體點出哪些個股偏多、哪些個股偏空，不要為了敘事簡潔而
只呈現對結論有利的那一檔個股。

**資金從哪裡流出？**
說明近30日明顯遭到資金撤退的板塊（如：科技成長股、半導體、生技等），原因為何？若提及具體個股，
請同時檢查該板塊2~3檔權值股是否方向一致。

**資金流向哪裡？**
說明近30日明顯吸引資金流入的板塊（如：金融、能源、公用事業、傳統製造等），驅動因素？若提及具體個股，
請同時檢查該板塊2~3檔權值股是否方向一致。
...
| 板塊名稱 | 資金方向 | 近30日表現 | 主要驅動力（若提及個股，需列2~3檔並標明方向是否一致） |
|------|------|------|------|
| 科技/AI | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
| 半導體 | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
| 金融 | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
| 能源 | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
| 公用事業 | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
| 傳統製造/工業 | 流入/流出/基本面待確認/板塊內部分化 | +X% / -X% | 說明 |
