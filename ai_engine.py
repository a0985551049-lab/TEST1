...
1. 目前指數相對於近14日高低點的相對位置（請直接引用上方提供的精確區間高低點與相對位置百分比，不需自行估算）
...
1. **加權指數（TAIEX）**：以上方系統數據為準，說明近14日高低點、目前位置、爬升或下跌動能是否減弱、成交量能變化
2. **櫃買指數（OTC）**：以上方系統數據為準，說明近14日走勢、目前位置、與加權指數的背離或共振
...
**極重要：「代表個股」務必與該類股的資金階段結論一致，不可只是隨便挑一檔知名龍頭股放上去。**
- 若某類股被歸類為「資金流入」或「確立流入」，所填入的代表個股本身近期股價與籌碼（外資/投信買賣超）
  也必須是同方向的（上漲或至少持穩、且有買超跡象），不能出現「類股寫資金流入，但代表個股其實股價下跌、
  且被外資賣超」這種自相矛盾的狀況。
- 若龍頭股本身近期其實正被外資獲利了結（股價下跌、籌碼賣超），即使族群整體基本面消息偏多，也「不可以」
  把它列為該類股的正向代表個股；應改選族群內籌碼真的偏多的個股，或在描述中明確點出「族群內部表態分化，
  龍頭股（如X）近期反而遭獲利了結、與族群整體資金水位不同步」，讓讀者清楚看到分歧，而不是含糊帶過。
...
5. **資金輪動路徑**：用「A類股 → B類股 → C類股」格式描述近14日資金水位輪動路徑，並試著標註目前資金正從哪個階段（流出/初步布局/確立流入）過渡到哪個階段。

接著請輸出一個「近14日類股資金流向總表」表格，**務必包含至少8行資料**，且必須涵蓋以上四種類別
（資金流出／資金流入／初步資金流入(外資布局)／仍受關注）都至少各出現1~2行，格式如下（每行都必須填入實際數據，嚴格按照此格式）：

| 類股名稱 | 資金階段 | 資金水位變化 | 主要驅動力/原因 | 代表個股（近14日表現） |
|------|------|------|------|------|
| 半導體 | 確立流入 | 水位持續墊高 | AI需求強勁，且外資近14日持續買超 | 台積電+8%，外資同步買超 |
| 金融股 | 流出探底 | 水位持續探底 | 升息預期降溫，外資調節 | 富邦金-3% |
| 電動車/綠能 | 初步布局 | 水位剛從低檔翻揚 | 外資近3日連續小額轉買超 | 某電動車概念股+2% |
| AI伺服器/ODM | 基本面待確認 | 尚無明確籌碼流向數據 | 6月營收年增逾五成，但查無近期外資買超佐證 | 龍頭股近期反遭外資獲利了結，族群內表態分化 |

其中「資金階段」欄位請從「確立流入」「初步布局」「持平觀察」「流出探底」「基本面待確認」五種標籤中擇一填入。
...
|------|------|------|------|
| 科技/AI | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
| 半導體 | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
| 金融 | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
| 能源 | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
| 公用事業 | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
| 傳統製造/工業 | 流入/流出/基本面待確認 | +X% / -X% | 說明 |
