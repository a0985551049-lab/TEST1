...
4. **國際市場連動**：近30日美股、亞股等國際市場的走勢，對台股同期表現造成了什麼程度的牽引或背離
   （例如：台股是否同步美股創高、或走出獨立於國際盤勢的自身格局），簡短說明即可

接著撰寫【第一節綜合結論】（至少100字），說明大盤目前的技術結構定性，以及是否存在「表面數字上漲但結構轉弱」或「表面下跌但已現止穩」的落差。

---

### 第二節：投資人氛圍與籌碼氣氛（至少500字）

本節聚焦「整體大盤層級」的投資人情緒與籌碼氣氛，**不需要、也不要分析個別類股或個股**，
重點是回答「現在市場整體是樂觀、謹慎、還是恐慌」，並說明判斷依據。

請搜尋並說明：

1. **融資融券水位**：全市場融資餘額近期是墊高還是下滑，代表散戶追價意願是升溫還是降溫；
   融券餘額變化是否顯示看空氣氛升溫

2. **外資/投信整體資金氣氛**：外資、投信近30日在台股整體（不分類股）的買賣超水位變化趨勢，
   是持續加碼、持續減碼、還是觀望，並說明這代表法人對台股整體的態度

3. **期貨與選擇權籌碼氛圍**：台指期未平倉量變化、選擇權Put/Call比（若能搜尋到），
   這些指標反映的是市場多空傾向偏樂觀、中性、還是偏保守

4. **成交量能與市場熱度**：近30日大盤成交量是放大還是萎縮，代表市場參與熱度上升或下降

接著撰寫【投資人氛圍綜合研判】（至少100字），用一句明確的定性描述目前市場氛圍
（例如：「樂觀但需留意過熱」「謹慎觀望，尚未出現明確方向」「恐慌情緒緩解，信心逐步回升」等），
並說明這個氛圍研判與第一節的指數技術結構是否互相呼應，或是出現「指數上漲但氣氛謹慎」之類的落差。
...
- 呼應第二節的投資人氛圍研判，總結目前市場情緒定性
- 下月關鍵事件
- 風險提示

---

**規範：** 阿拉伯數字、避免投資指示用語、僅供學術研究、總字數1400字以上、只需輸出第一、二、三節，不要輸出個股或類股推薦內容
"""
...
def analyze_us_market(api_key, chart_data=None):
    """
    使用 Gemini + Google Search Grounding 分析近30日美股總體經濟、市場氛圍與對台股的影響
    涵蓋：費城半導體、那斯達克、標普500、美元指數、VIX、
          10年期公債殖利率、CPI、非農就業、失業率、FED利率

    單次呼叫 Gemini 產生完整報告（五節）。
    ...
    try:
        ...
        prompt = f"""
今天是 {today}。請搜尋最新美股與總體經濟資訊，用繁體中文撰寫一份「美股總經雷達月報」，共五節，請完整輸出全部五節。
...
**重要規範：本節「禁止」輸出任何表格（不可出現任何 `|` 符號的表格列），只能用文字段落＋條列說明呈現。**
...
### 第五節：本月美股總經全面結論與操作建議（至少300字）

這是本報告的總結，必須完整輸出，不可省略：

**整體市場定性：**
綜合以上四節所有分析，用3-5句話說明目前美股市場的整體健康狀態與主要驅動因素。

**攻守勢最終裁定：**
明確寫出「本月美股整體判斷為：【攻勢型】」或「本月美股整體判斷為：【守勢型】」，並說明判斷依據
（攻勢型：市場偏好追逐高Beta成長股，如科技/半導體/AI概念；守勢型：資金轉趨保守，偏好防禦性資產）。

**最大風險警示：**
列出目前最需警惕的1-3個市場風險（如：公債殖利率飆升、VIX突破警戒線、美元急升壓縮新興市場等）。

**對台股投資人的建議方向：**
根據美股總經環境與投資人氛圍，給出台股投資人應重點關注的方向（如：台積電跟隨費半、金融股受惠降息、防禦型配置等）。僅供學術研究參考。

---

**規範：** 全文使用阿拉伯數字、繁體中文、僅供學術研究、數據務必標明來源日期、每一節都必須完整輸出不可中途停止、總字數2000字以上、只需輸出第一至第五節，不要有開場白或結語。
"""
